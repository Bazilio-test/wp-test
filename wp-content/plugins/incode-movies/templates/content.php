    ...
    <footer class="entry-meta">
		...
		if($terms = wp_get_post_terms(get_the_ID(), 'taxcountries', ["fields" => "all"])){
			if(!is_wp_error($terms)){
				$meta_countries = '<span class="incode-movies-l">' . __(count($terms) == 1 ? 'Country' : 'Countries', 'incode-movies') . ':</span><span class="incode-movies-d">';
		...
		if($terms = wp_get_post_terms(get_the_ID(), 'taxgenres', ["fields" => "all"])){
			if(!is_wp_error($terms)){
				$meta_genres = '<span class="incode-movies-l">' . __(count($terms) == 1 ? 'Genre' : 'Genres', 'incode-movies') . ':</span><span class="incode-movies-d">';
				foreach($terms as $term){
